CREATE OR REPLACE FUNCTION public.get_fan_ranking()
 RETURNS TABLE(user_id uuid, full_name text, avatar_url text, favorite_club_id text, sessions_count integer, checkins_count integer, courses_count integer, points integer)
 LANGUAGE sql
...
AS $function$
  ...
  SELECT
    ...
    (COALESCE(s.c, 0) * 3 + COALESCE(e.c, 0) + COALESCE(c.c, 0))::int AS points
  FROM public.profiles p
  LEFT JOIN s ON s.user_id = p.user_id
  LEFT JOIN e ON e.user_id = p.user_id
  LEFT JOIN c ON c.user_id = p.user_id
  WHERE (COALESCE(s.c, 0) + COALESCE(e.c, 0) + COALESCE(c.c, 0)) > 0
