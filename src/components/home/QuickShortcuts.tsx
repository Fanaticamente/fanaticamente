...
const ShortcutCard = ({ to, icon, title, subtitle }: ShortcutProps) => (
  ...
  >
    <div className="relative overflow-hidden rounded-2xl bg-[#0e0e0e] px-3 py-4 flex items-center gap-2.5">
      {/* top glow */}
  ...
  return (
    <div className="px-4 py-4 grid grid-cols-2 gap-3">
      <ShortcutCard
        ...
        icon={<Calendar className="w-7 h-7 text-emerald-500 shrink-0" strokeWidth={2} />}
        title="Minhas Consultas"
        subtitle="Gerencie atendimentos"
      />
      <ShortcutCard
        to="/meus-cursos"
        icon={<GraduationCap className="w-7 h-7 text-emerald-500 shrink-0" strokeWidth={2} />}
        title="Meus Cursos"
