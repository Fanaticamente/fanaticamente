    ...
    <div className="space-y-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1.5">
          ...
          <Input value={cardNumber} onChange={(e) => setCardNumber(formatCardNumber(e.target.value))} placeholder="0000 0000 0000 0000" maxLength={19} required className="bg-background border-border" />
        </div>

        <div className="space-y-1.5">
          <Label className="text-sm text-muted-foreground">Nome no cartão</Label>
          <Input value={cardholderName} onChange={(e) => setCardholderName(e.target.value)} placeholder="NOME COMO NO CARTÃO" required className="bg-background border-border uppercase" />
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1.5">
            ...
            <Select value={expirationMonth} onValueChange={setExpirationMonth} required>
              <SelectTrigger className="bg-background border-border"><SelectValue placeholder="MM" /></SelectTrigger>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={expirationYear} onValueChange={setExpirationYear} required>
              <SelectTrigger className="bg-background border-border"><SelectValue placeholder="AAAA" /></SelectTrigger>
              <SelectContent>{years.map(y => <SelectItem key={y} value={y}>{y}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
            <Label className="text-sm text-muted-foreground">CVV</Label>
            <Input value={securityCode} onChange={(e) => setSecurityCode(e.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="123" maxLength={4} required className="bg-background border-border" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="col-span-2 space-y-1.5">
            ...
            <Input value={docNumber} onChange={(e) => setDocNumber(formatDocNumber(e.target.value))} placeholder={docType === "CPF" ? "000.000.000-00" : "00.000.000/0000-00"} required className="bg-background border-border" />
          </div>
        </div>
        ...
        <button type="submit" disabled={isLoading || !isSDKLoaded} className="w-full py-4 bg-amber-600 text-white rounded-xl font-bold uppercase tracking-wide hover:bg-amber-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
